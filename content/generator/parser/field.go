...
func (p *parser) ParseField(raw string, gt *types.TypeDefinition) (*types.Field, error) {
	// contents:string
	...
	if !strings.Contains(raw, ":") {
		return nil, fmt.Errorf("invalid parser argument. [%s]", raw)
	}
	...
	p.setFieldTypeName(field, data[1], gt)
	viewType := "input"
	if len(data) == 3 {
		viewType = data[2]
	}
...
func (p *parser) setFieldTypeName(field *types.Field, fieldType string, gt *types.TypeDefinition) {
	...
	if _, ok := p.types.Content[field.ReferenceName]; ok {
		field.IsReference = true
		gt.HasReferences = true
	} else if _, ok = p.types.FieldCollections[field.ReferenceName]; ok {
