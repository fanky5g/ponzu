...
var dom = (function() {
  ...
  var range = (function() {
    ...
    var WrappedRange = function(sc, so, ec, eo) {
      ...
      /**
       * create offsetPath bookmark root on paragraph
       *
       * @param {Node[]} paras
       */
  ...
  */
    ...
    lang: {
      'en-US': {
        ...
        image: {
          ...
          selectFromFiles: 'Select from uploads',
  ...
  var Editor = function(handler) {
    ...
    this.insertImage = function($editable, sUrl, filename) {
      async.createImage(sUrl, filename).then(function($image) {
        ...
        var $holder = dom.makeLayoutInfo($editable).holder();
        handler.bindCustomEvent(
          $holder, $editable.data('callbacks'), 'image.storage.error'
        )();
      });
    };
  ...
  var LinkDialog = function(handler) {
    ...
    /**
     * Show link dialog and set event controllers on dialog controls.
     *
     * @param {jQuery} $editable
     * @param {jQuery} $dialog
     * @param {Object} linkInfo
  ...
  var EventHandler = function() {
    ...
    this.insertImages = function(layoutInfo, files) {
      ...
      if (callbacks.onImageUpload) {
        bindCustomEvent($holder, callbacks, 'image.storage')(files);
      // else insert Image as dataURL
      } else {
        $.each(files, function(idx, file) {
          var filename = file.name;
          if (options.maximumImageFileSize && options.maximumImageFileSize < file.size) {
            bindCustomEvent($holder, callbacks, 'image.storage.error')(options.langInfo.image.maximumFileSizeError);
          } else {
     ...
     *
     * @param {Object} layoutInfo - layout Informations
     * @param {Object} options - user options include custom event controllers
     */
    this.attach = function(layoutInfo, options) {
      // controllers for editable
      ...
      // controllers for frame mode (toolbar, statusbar)
      if (!options.airMode) {
        // handler for toolbar
                ...
                '<div class="col s12">' +
                    '<div class="file-field input-field">' +
                            '<div class="btn">' +
                                ...
                                '<input class="note-image-input" name="uploads" type="file" />' +
                            '</div>' +
